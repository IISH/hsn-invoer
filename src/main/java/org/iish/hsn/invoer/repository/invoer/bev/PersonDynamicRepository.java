...
public interface PersonDynamicRepository extends Repository<PersonDynamic, Integer> {
    @Query("SELECT pd FROM PersonDynamic pd " +
           ...
           "AND pd.dynamicDataType = ?3 AND pd.dynamicDataSequenceNumber = 1 AND pd.workOrder = ?4")
    public PersonDynamic findFirstPersonDynamicForPerson(RegistrationId registrationId, int keyToRegistrationPersons,
    ...
    public List<PersonDynamic> findAllPersonDynamicForPerson(RegistrationId registrationId,
                                                             int keyToRegistrationPersons, WorkOrder workOrder);

    public List<PersonDynamic> save(Iterable<PersonDynamic> entities);
